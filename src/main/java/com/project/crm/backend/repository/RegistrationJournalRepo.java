...
public interface RegistrationJournalRepo extends JpaRepository<RegistrationJournal, Long> {
    ...
    @Query(value = "select * from users u, registrations_journal rj where u.id = rj.user_id and rj.role_id = 5  and rj.hospital_id = ?1 order by u.full_name asc", nativeQuery = true)
    Page<RegistrationJournal> findAllHospitalPatients(Long hospitalId, Pageable pageable);
